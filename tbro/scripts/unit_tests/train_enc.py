...
def validate(test_loader, full_errors=False):
  test_loss = []
  ...
  for i, data in enumerate(test_loader):
    ...
    for j in range(seq_len):
        radar_data[j] = radar_data[j].to(device)
        if j < seq_len-1:
            positions[j] = positions[j].to(device)
            orientations[j] = orientations[j].to(device)
        if torch.any(torch.isnan(radar_data[j])):
            print('Image'+str(j)+' contains NaN')

    with torch.no_grad():
        y_hat = model(radar_data)
    

    position_loss = loss_func(y_hat[:,3:], positions[0])
    orientation_loss = loss_func(y_hat[:,:3], orientations[0])
    loss = 1 * orientation_loss + position_loss

    test_loss.append(loss.item())

    position_err_batch = (y_hat[:,3:] - positions[0]).abs().view(-1,3)
    
    orientation_err_batch = (y_hat[:,:3] - orientations[0]).abs().view(-1,3)
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(train_dataset,batch_size=1,shuffle=True,num_workers=8,drop_last=False)
    ...
    if not args.eval_only:
        ...
        for epoch in range(num_epochs):
            ...
            if (epoch < num_epochs - 1):
                model = model.eval()
                test_losses = validate(test_loader)
    ...
    model = model.eval()
    test_loss = validate(test_loader, True)
    ...
    torch.save(epoch_test_losses, 'test_losses.pt')
    torch.save(epoch_train_losses, 'train_losses.pt')
    writer.close()
