...
from scripts.models.deepro_transformer_reduced_enc import DeepROTransformer
...
class rpe_callback(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if self.traj_gt.size()[0] >= 500:
            x = self.traj_gt[500,0:self.args.max_length,0].squeeze()
            y = self.traj_gt[500,0:self.args.max_length,1].squeeze()
            x = x.cpu()
            y = y.cpu()
            x = x.numpy()
            y = y.numpy()

            x_hat = self.traj_est[500,0:self.args.max_length,0].squeeze()
            y_hat = self.traj_est[500,0:self.args.max_length,1].squeeze()
            x_hat = x_hat.cpu()
            y_hat = y_hat.cpu()
            x_hat = x_hat.numpy()
            y_hat = y_hat.numpy()
